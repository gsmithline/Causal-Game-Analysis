...
from rl_training.algorithms.impala.storage import collate_rollout
from rl_training.algorithms.impala.vtrace import compute_vtrace
from rl_training.algorithms.impala.config import ImpalaConfig


class Learner:
    """IMPALA learner that updates policy using V-trace."""
    ...
    def step(self) -> Dict[str, Any]:
        ...
        """
        rollouts = [self.queue.get() for _ in range(self.config.batch_size)]
        ...
        loss = (
            pg_loss
            + self.config.value_coef * baseline_loss
