...
As AI agents are increasingly deployed in shared environments, individually capable agents do not guarantee good system-level outcomes. Even if each agent is well-designed, the *equilibrium* of the resulting game — which predicts how the agents will be used — can be welfare-destroying (Conitzer & Oesterheld 2023; Dafoe et al. 2020). The composition of the agent population determines which equilibrium forms, and different compositions can lock the system into cooperative or competitive regimes.

In the metagame, each agent corresponds to a strategy available to players. An equilibrium is a prediction of how players will mix over these strategies. Standard metagame analysis (EGTA) evaluates agents by how they perform *at* equilibrium. We ask a different question: **what does each agent do *to* the equilibrium prediction?** Adding an agent to the strategy set changes the game, which changes the equilibrium, which changes the outcome for everyone. LOO (leave-one-out) analysis measures this directly: how does the equilibrium prediction — and the welfare it implies — change when a given agent is or isn't available?

An agent can be individually strong but shift the equilibrium toward a worse outcome for the system. Conversely, an agent that is never played at equilibrium can still shape which equilibrium forms by its presence in the strategy set. Equilibrium is a nonlinear function of the strategy set, so the impact of including an agent is generally not predictable from its performance within the equilibrium. This can reveal structural roles invisible to standard analysis: which agents are holding the current equilibrium in place, which would shift it to a qualitatively different one, and which are substitutable.

### Interpreting the Counterfactual Measures

An equilibrium of the metagame can be seen as a **prediction** of how players would use the available strategies. But the strategy population in an empirical metagame is ad hoc: it is whatever collection of agents researchers or practitioners happened to build. There is no canonical strategy set, so the prediction depends on the composition of the strategy pool. The LOO decomposition is not prescriptive — it does not recommend removing strategies. Rather, it is a **diagnostic tool for understanding what drives the equilibrium prediction**, analogous to how ANOVA decomposes variance into attributable factors without recommending that any factor be eliminated. L2 decomposes the equilibrium into individual strategy contributions. L3 (Harsanyi interaction dividends) captures non-additive effects — cases where the contribution of one strategy depends on the presence of another. Together they reveal which parts of the equilibrium prediction are robust to the composition of the strategy set and which are fragile.

Each strategy, by being available as a choice in the metagame, imposes **externalities** on the system. Nash equilibrium depends on off-equilibrium threats: the set of available deviations defines which profiles are stable. A strategy with zero equilibrium support can still be the most consequential agent in the game if its availability as a credible deviation prevents a better equilibrium from forming.

**Level 2 (full-game LOO)** measures the externality: how much does making this strategy available change equilibrium welfare? This is not a measure of individual performance but of system-level consequence. In the bargaining domain, 5.2_low is never played but imposes a negative externality of 10.87 UW and 7.7% EF1 — it suppresses a cooperative equilibrium by serving as an off-equilibrium threat against 5.4_medium. Meanwhile PPO, the 79% equilibrium agent, has near-zero externality because MAPPO substitutes for it perfectly.

**Level 3 (CURB-conditional LOO)** measures whether the externality is robust or context-dependent. An agent's system-level impact can vary across strategically coherent subgames. 5.2_low's externality ranges from -46.92 to 0.00 UW depending on the strategic context — catastrophic in some CURB sets, irrelevant in others. This distinguishes agents that are structurally harmful from agents whose impact is contingent on who else is present.

Together, L2 and L3 provide an evaluation of agents by their system-level consequences rather than their individual performance: what externalities does each agent impose on the metagame, and are those externalities robust?
