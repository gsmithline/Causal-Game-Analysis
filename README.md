...
Individually capable agents do not guarantee good system-level outcomes. Even if each agent is well-designed, the equilibrium of the resulting game can be welfare-destroying (Conitzer & Oesterheld 2023; Dafoe et al. 2020). The composition of the agent population determines which equilibrium forms, and different compositions can lock the system into cooperative or competitive regimes. An agent can be individually strong but shift the equilibrium prediction toward a worse outcome for the system. Conversely, an agent that is never played can still shape which equilibrium forms by its presence in the strategy set.
