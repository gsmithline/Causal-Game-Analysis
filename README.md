...
Empirical game-theoretic analysis (EGTA) constructs a game model over a population of agents by simulating head-to-head interactions, estimating payoffs, and computing equilibria (Wellman et al. 2025; Li & Wellman 2024). In the metagame, each agent corresponds to a strategy available to players, and an equilibrium is a **prediction** of how the agents will be used. Smithline et al. (2025) apply this framework to LLM bargaining, evaluating agents at equilibrium on individual effectiveness (regret), social welfare, and fairness. This tells you how agents perform *at* the predicted equilibrium — but not what each agent does *to* the prediction itself.

We extend this framework with counterfactual analysis: **what does each agent do *to* the equilibrium prediction?**

### Cooperative AI framing

"Even if each individual agent is almost perfectly aligned with our objective, it is possible that the only equilibrium of the resulting game results in a terrible outcome. Aligning individual agents is not sufficient for the multiagent system as a whole to be well aligned with our objectives, due to game-theoretic phenomena" (Conitzer & Oesterheld 2023). As AI agents are increasingly deployed in shared environments, we need to understand not just individual agent quality but how each agent affects the system-level equilibrium outcome (Dafoe et al. 2020). An agent that achieves the highest welfare in head-to-head play can still, by its presence in the strategy set, prevent the system from reaching a higher-welfare equilibrium. The composition of the agent population determines which equilibrium forms, and different compositions can lock the system into cooperative or competitive regimes. Our counterfactual analysis measures each agent's contribution to whether the system reaches a good or bad equilibrium — diagnosing the problem that Conitzer & Oesterheld identify.

### Designer framing

The strategy population in an empirical metagame is ad hoc — it is whatever collection of agents researchers or practitioners happened to build. There is no canonical strategy set, so the equilibrium prediction depends on who is in the pool. Each agent, by being available as a choice, imposes **externalities** on the system: its inclusion changes the game, changes the equilibrium, and changes the outcome for everyone. Nash equilibrium depends on off-equilibrium threats — the set of available deviations defines which profiles are stable — so an agent with zero equilibrium support can still be the most consequential member of the population if its availability as a credible deviation prevents a better equilibrium from forming.

### Diagnostic framing

LOO (leave-one-out) analysis measures each agent's impact on the equilibrium prediction directly: how does the predicted welfare change when a given agent is or isn't available? The LOO decomposition is not prescriptive — it does not recommend removing agents. Rather, it is a **diagnostic tool for understanding what drives the equilibrium prediction**, analogous to how ANOVA decomposes variance into attributable factors without recommending that any factor be eliminated. A high positive LOO effect (removing the agent improves welfare) means the system reaches a better equilibrium without this agent. A high negative effect means the system needs this agent. Near-zero means the agent is substitutable or irrelevant.
