...
Nash equilibrium violates independence of irrelevant alternatives (IIA): removing a strategy outside the equilibrium support can change which equilibrium is selected entirely. Consider a metagame where three RL agents dominate the equilibrium. Adding an LLM agent that receives zero equilibrium weight may nonetheless shift the equilibrium to one with higher welfare, because the LLM's presence changes the best-response structure for the RL agents. Standard EGTA cannot detect this. It reports the equilibrium but not which agents are load-bearing for it.

This matters because a strategy that does not rank highest on any individual metric may nonetheless drive higher cooperation or welfare at equilibrium through its strategic interactions with other agents. As LLMs are increasingly deployed as autonomous agents in strategic settings, understanding how their presence affects cooperation and welfare in multi-agent populations is of growing importance.

As multi-agent competitions scale, from Kaggle's [Game Arena](https://www.kaggle.com/game-arena) where LLMs compete in Werewolf, Poker, and Chess, to PSRO-trained leagues like AlphaStar, practitioners need to know not just which agents win, but which agents' presence shapes the equilibrium. Measuring cooperation and social welfare in multi-agent systems has been a longstanding goal, with various metrics proposed for quantifying individual agent behavior. We contribute a different methodology: rather than measuring cooperation as a property of individual agents, we measure each agent's *causal contribution* to cooperation and welfare at equilibrium, asking not "how cooperative is this agent?" but "how much does this agent's presence cause the equilibrium to be more cooperative?"
